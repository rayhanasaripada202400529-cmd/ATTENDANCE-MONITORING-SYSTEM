...
function readData() {
  if (!fs.existsSync(DATA_FILE)) {
    fs.writeFileSync(
      DATA_FILE,
      JSON.stringify({ users: [], students: [], attendance: [] })
    );
  }
...
app.post('/api/attendance', (req, res) => {
  ...
  if (!student_id || !status) return res.status(400).json({ message: 'student_id and status required' });
  const data = readData();
...
app.put('/api/attendance/:id', (req, res) => {
  const data = readData();
  const index = data.attendance.findIndex(a => a.id === req.params.id);
  if (index === -1) return res.status(404).json({ message: 'Attendance record not found' });
  const { status } = req.body;
  if (!status) return res.status(400).json({ message: 'status is required to update attendance' });
...
app.delete('/api/attendance/:id', (req, res) => {
  ...
  if (recordIndex === -1) return res.status(404).json({ message: 'Attendance record not found' });
...
app.get('/api/attendance/summary/:studentId', (req, res) => {
  ...
  let present = 0;
  let absent = 0;
  records.forEach(r => { if (r.status === 'present') present++; else absent++; });
  const percentage = records.length ? ((present / records.length) * 100).toFixed(2) : 0;
  let alert = absent >= 3 ? 'Warning: Frequent absences detected!' : null;
  res.json({ total_present: present, total_absent: absent, attendance_percentage: percentage, alert });
});
